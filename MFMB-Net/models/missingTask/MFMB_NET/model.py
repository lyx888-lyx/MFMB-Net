...
class RECLoss(nn.Module):
    ...
    def forward(self, pred, target, mask):
        ...
        weight_sim = getattr(self.args, 'weight_sim_loss', 0)
        if self.args.recloss_type == 'combine' and weight_sim != 0:
            loss += (weight_sim * self.loss_cmd(pred * mask, target * mask) / (torch.sum(mask) + self.eps))
        return loss
...
class MFMB_NET(nn.Module):
    ...
    def forward(self, text, audio, vision, labels=None, epoch=1, training=False):
        ...
        if not self.args.without_generator:
            ...
            fusion_out = self.fusion_subnet(
                (text_h, text_mask),
                (audio_h, audio_mask),
                (vision_h, vision_mask),
                mide_context=mide_context,
            )
            if isinstance(fusion_out, tuple):
                prediction, mide_aux = fusion_out
            else:
                prediction, mide_aux = fusion_out, None
            ...
            return prediction, gen_loss, mide_aux
        ...
        if isinstance(fusion_out, tuple):
            ...
            prediction, mide_aux = fusion_out, None
        return prediction, torch.Tensor([0]).to(self.args.device), mide_aux
