...
class GRUencoder(nn.Module):
    ...
    def forward(self, utterance, utterance_lens):
        ...
        utterance_output = pad_packed_sequence(
            utterance_output,
            total_length=utterance.size(1)
        )[0]
...
class C_GATE(nn.Module):
    ...
    def forward(self, utterance, utterance_mask):
        ...
        add_zero = torch.zeros(
            size=[utterance.shape[0], 1],
            requires_grad=False
        ).type_as(utterance_mask).to(utterance_mask.device)
        ...
        gate = torch.sigmoid(
            self.cnn(transformed_.transpose(1, 2)).transpose(1, 2)
        )  # [batch_size, seq_len, 1]
        gate_x = torch.tanh(transformed_) * gate  # [batch_size, seq_len, 2 * hidden_dim]

        utterance_rep = torch.tanh(
            self.fc(torch.cat([utterance, gate_x], dim=-1))
...
class GATE_F(nn.Module):
    def __init__(self, args):
        ...
        # 数据集名称：决定 forward 里使用 mosi 还是 mosei 的线性层
        self.dataset_name = args.datasetName.lower()
        ...
        self.classifier2.add_module(
            'linear_trans_norm',
            nn.BatchNorm1d(args.fusion_t_hid + args.fusion_a_hid + args.fusion_v_hid + self.common_size * 4 + 90)
        )
        self.classifier2.add_module(
            'linear_trans_hidden',
            nn.Linear(
                args.fusion_t_hid + args.fusion_a_hid + args.fusion_v_hid + self.common_size * 4 + 90,
                args.cls_hidden_dim
            )
        )
    ...
    def forward(self, text_x, audio_x, vision_x):
        ...
        current_batch_size = text_x.size(0)

        # text
        text_x_fusion = text_x.permute(1, 0, 2)  # [seq, batch, dim]

        # audio / vision
        # 这里是最关键的修复：MOSEI 不能再错误走 MOSI 的线性层
        ...
        audio_x_fusion = audio_x_fusion.permute(2, 0, 1)   # [seq, batch, dim]
        vision_x_fusion = vision_x_fusion.permute(2, 0, 1) # [seq, batch, dim]

        audio_visual_fusion = self.audio_visual_model(audio_x_fusion, vision_x_fusion, text_x_fusion)
        audio_visual_fusion = audio_visual_fusion[-1]  # [batch, 90]
        ...
        stack_rep_ta = torch.stack((text_rep_common, audio_rep_common), dim=0)   # [2, batch, common_size]
        stack_rep_tv = torch.stack((text_rep_common, vision_rep_common), dim=0)  # [2, batch, common_size]

        stack_rep_ta = self.batchnorm(stack_rep_ta.permute(1, 0, 2))  # [batch, 2, common_size]
        stack_rep_tv = self.batchnorm(stack_rep_tv.permute(1, 0, 2))  # [batch, 2, common_size]

        # 原代码写死 self.batch_size，这里改成当前 batch 的真实大小，更稳
        stack_rep_tv = stack_rep_tv.reshape(current_batch_size, -1)
        stack_rep_ta = stack_rep_ta.reshape(current_batch_size, -1)
